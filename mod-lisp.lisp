;;; mod_lisp copyright 2000-20014 Marc Battyani see license.txt for the details
;;; You can reach me at marc.battyani@fractalconcept.com or marc@battyani.com
;;; The source code repository for mod_lisp is here: https://github.com/mbattyani/mod_lisp

(require "comm")

(defconstant +apache-port+ 3000)
(defvar *apache-socket* nil "the socket connection to apache")
(defvar *close-apache-socket* nil
  "set to t if you want to close the socket to apache after sending each page content")
(defvar *apache-nb-requests-socket* 0
  "the number of requests sent through this socket")
...
(defun apache-listen (*apache-socket*)
  (let ((*close-apache-socket* t))
    (unwind-protect
      (loop for *apache-nb-requests-socket* from 0
...
<TR bgcolor=\"yellow\"><TH>Key</TH><TH>Value</TH></TR>" s)
   (format s "<TR bgcolor=\"#F0F0c0\"><TD>apache-nb-requests-socket</TD><TD>~a</TD></TR>"  *apache-nb-requests-socket*)
